...
async function searchBlood(params = {}) {
  const bloodType = params.type || params.bloodType;
  ...
  const radiusKm = parseFloat(params.radius || params.radiusKm) || 50;
  const radiusMeters = radiusKm * 1000;

  const orgs = await orgRepo.findNearby({
    bloodType,
    lat: latitude,
    lng: longitude,
    radiusMeters,
