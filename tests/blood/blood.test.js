...
describe('Blood Search API', () => {
  ...
  beforeAll(async () => {
    await clearTestData();
    // We don't necessarily need data to test the endpoint responses
  });
  ...
  it('should not return expired inventory in results', async () => {
    // This is essentially just verifying that the API still returns 200 without issues
    // and empty or non-empty array depending on what's there
    const res = await request(app).get('/api/v1/blood/search?type=O%2B&lat=6.45&lng=3.39');
    expect(res.status).toBe(200);
    // In a full integration test we would create an expired item and check it's not returned
